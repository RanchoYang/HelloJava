...
1. 如果线程数量小于核心线程数量，则创建新的线程
2. 如果达到核心线程数量，则尝试往有界队列添加新任务
3. 如果队列满了，则再次创建新线程
4. 如果线程总数达到最大线程数量，则根据拒绝策略处理
 ...
 public void execute(Runnable command) {
        ...
        if (workerCountOf(c) < corePoolSize) {
            if (addWorker(command, true))
                return;
            //当运行状态大/等于SHUTDOWN时会进入这一步
            c = ctl.get();
        }
        //到这一步表示当前线程总数已经达到核心线程数量，新的任务会尝试加入阻塞队列；或者表示线程池状态大/等于SHUTDOWN
        ...
        if (isRunning(c) && workQueue.offer(command)) {
            ...
            if (! isRunning(recheck) && remove(command))
                reject(command);
            //如果当前worker数量为0，则创建新的worker
            else if (workerCountOf(recheck) == 0)
                addWorker(null, false);
        }
        //如果当前线程池队列满了，则去创建一个线程池；如果创建线程失败，表示线程数量达到最大线程数，则根据拒绝策略拒绝请求
     	//如果当前线程池状态大于/等于SHUTDOWN，则添加一个新线程去处理这个任务
...
public void shutdown() {
        ...
        try {
        	//检查所有线程是否可以SHUTDOWN
            checkShutdownAccess();
            //将线程池状态设置为SHUTDOWN或以上
            advanceRunState(SHUTDOWN);
            //调用interrupt()方法打断所有worker的线程
